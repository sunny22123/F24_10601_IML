...
def zero_init(shape: Tuple[int, int]) -> np.ndarray:
    """
    Initializes a numpy array with shape `shape` with zeros.
    :param shape: The shape of the numpy array
    :return: Numpy array with the specified shape, filled with zeros
    """
...
class SoftMaxCrossEntropy:
    ...
    def _cross_entropy(self, y: int, y_hat: np.ndarray) -> float:
        ...
        """
        # TODO: implement
        # y_hat[y] is the predicted probability for the true class
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> np.ndarray:
        ...
        dz = y_hat.copy()
        dz[y] -= 1  # Subtract 1 from the true class's probability
        return dz
...
class Sigmoid:
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO: implement
        sigmoid_derivative = self.sigmoid_output * (1 - self.sigmoid_output)
...
class Linear:
    def __init__(self, input_size: int, output_size: int,
        ...
        self.w = weight_init_fn((output_size, input_size + 1))  # +1 for bias

        # Initialize bias terms to zero (it's folded into the weight matrix)
        self.w[:, -1] = 0.0

        # Initialize matrix to store gradient of weights (dw)
        self.dw = np.zeros_like(self.w)

        # Cache the input x for use in backpropagation
        self.x = None
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        # TODO: perform forward pass and save any values you may need for
        #  the backward pass
        x_with_bias = np.append(x, 1)  # Add bias term to input
        self.x = x_with_bias  # Cache the input for backpropagation

        # Perform forward pass (linear transformation)
        z = self.w @ x_with_bias  # Matrix multiplication
        return z
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        your forward() method.
        """
        self.dw = np.outer(dz, self.x)  # dz has shape (output_size,), x has shape (input_size + 1,)

        # Compute dx (gradient w.r.t input) - note we ignore the bias term
        dx = self.w[:, :-1].T @ dz  # Matrix multiplication with weights excluding bias
        return dx
    ...
    def step(self) -> None:
        ...
        """
        # TODO: implement
        self.w -= self.lr * self.dw  # Update weights using SGD
...
class NN:
    def __init__(self, input_size: int, hidden_size: int, output_size: int,
                 weight_init_fn: INIT_FN_TYPE, learning_rate: float):
        """
        Initialize neural network (NN) class.
    ...
    def forward(self, x: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        """
        # Layer 1: Linear -> Sigmoid -> Linear
        out1 = self.linear1.forward(x)
        out2 = self.sigmoid.forward(out1)
        logits = self.linear2.forward(out2)

        # Compute softmax and cross-entropy loss
        y_hat, loss = self.loss_fn.forward(logits, y)
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> None:
        ...
        """
        # Compute gradient of loss w.r.t. softmax input
        loss_grad = self.loss_fn.backward(y, y_hat)

        # Backprop through the layers
        grad2 = self.linear2.backward(loss_grad)
        grad1 = self.sigmoid.backward(grad2)
        self.linear1.backward(grad1)
    ...
    def compute_loss(self, X: np.ndarray, y: np.ndarray) -> float:
        """
        Compute the neural network's average cross-entropy loss over the dataset (X, y)
        ...
        for x_sample, y_sample in zip(X, y):
            _, loss = self.forward(x_sample, y_sample)
            total_loss += loss
    ...
    def train(self, X_tr: np.ndarray, y_tr: np.ndarray,
        ...
        :return:
            train_losses: Training losses after each epoch
            test_losses: Test losses after each epoch
        """
        train_losses = []
        test_losses = []

        # Keep a copy of the original training data to reshuffle it every epoch
        original_X_tr = np.copy(X_tr)
        original_y_tr = np.copy(y_tr)

        for epoch in range(n_epochs):
            # Reshuffle the original training data (not the shuffled data from previous epoch)
    ...
    def test(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        error = 0
        for x_sample, y_sample in zip(X, y):
            y_hat, _ = self.forward(x_sample, y_sample)
