...
def plot_returns(total_returns, episodes, label, rolling_window=25):
    rolling_mean = pd.Series(total_returns).rolling(window=rolling_window).mean()
    plt.plot(range(episodes), total_returns, label=f"{label} Returns")
    plt.plot(range(episodes), rolling_mean, label=f"{label} Rolling Mean (window={rolling_window})")
    plt.xlabel("Episode")
...
if __name__ == "__main__":
    ...
    plot_returns(total_returns, episodes, mode)
    plt.title("Q-Learning Performance on Mountain Car")
    plt.savefig(f"performance_{mode}.png")
    plt.show()
