...
if __name__ == "__main__":
    ...
    lm = torch.load("/Users/leesunny/PycharmProjects/10601 ML/hw7/hw7-4/handout/model.pt")
    lm = lm.to(device)
    lm.eval()

    # Test prefix
    test_prefix = "Once upon a time there was a"

    # Loop through temperature values
    for temperature in [0, 0.3, 0.8]:
        print(f"--- Generating text with temperature {temperature} ---")
        completion = complete(test_prefix, num_tokens=64, temperature=temperature)
